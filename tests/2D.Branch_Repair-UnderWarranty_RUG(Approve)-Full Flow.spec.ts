...
test('Repair RUG', async ({ page }, testInfo) => {
  ...
  // Give the SO form's header action buttons a moment to finish rendering
  // (they can lag slightly behind waitForLoading's idle signal), then poll
  // a few times before concluding an optional button truly isn't present —
  // a single short waitFor can false-negative on a slow render.
  await page.waitForTimeout(500);
  const waitVisibleWithRetry = async (
    locator: Locator,
    attempts = 3,
    perAttemptMs = 8_000,
  ): Promise<boolean> => {
    for (let i = 0; i < attempts; i++) {
      if (await locator.waitFor({ state: 'visible', timeout: perAttemptMs }).then(() => true).catch(() => false)) {
        return true;
      }
      await waitForLoading(page);
      await page.waitForTimeout(500);
  ...
  if (await waitVisibleWithRetry(requestRugBtn)) {
  ...
  if (await waitVisibleWithRetry(approveRugBtn)) {
  ...
  if (await waitVisibleWithRetry(confirmBtn)) {
  ...
  // Single delivery order — the Delivery button may have opened its form
  // directly, OR landed on a list containing exactly one row. Either way,
  // open it (if needed) and validate it the same way as each row in the
  // multi-delivery loop below, then close it with the "×" button.
  let singleDeliveryProcessedCount = 0;
  if (totalDeliveries === 1) {
    ...
    const singleValidateBtn = page.locator('button').filter({ hasText: /^Validate$/i }).first();
    if (await singleValidateBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await singleValidateBtn.click();
      await waitForLoading(page);
      const singleImmTransferBtn = page.locator('.o_dialog button').filter({ hasText: /Immediate Transfer|^Validate$/i }).first();
      if (await singleImmTransferBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
        await singleImmTransferBtn.click();
        await waitForLoading(page);
      }
      const singleNoBackorderBtn = page.locator('.o_dialog button').filter({ hasText: /No Backorder/i }).first();
      if (await singleNoBackorderBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
        await singleNoBackorderBtn.click();
        await waitForLoading(page);
      }
      await expect(
      ...
      singleDeliveryProcessedCount = 1;

      const singleCloseXBtn = page.locator('.o_dialog [aria-label="Close"], .o_dialog .btn-close, [aria-label="Close"], .btn-close').first();
      if (await singleCloseXBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
        await singleCloseXBtn.click();
        await waitForLoading(page);
  ...
  if (totalDeliveries > 1 || singleDeliveryProcessedCount > 0) {
    ...
    let processedCount = singleDeliveryProcessedCount;
    let more = true;
    while (more) {
      more = false;
