...
test('Branch Repair - Under Warranty (External not RUG) - Quick Repair', async ({ page, context }, testInfo) => {
  ...
  // Select "Repair - Under Warranty -  External not RUG" from the Type field
  // (fillMany2one already waits for its own onchange RPC via waitForLoading)
  await fillMany2one(page, 'ticket_type_id', 'Repair - Under Warranty -  External not RUG');
  console.log('  ✓ Type set to: Repair - Under Warranty -  External not RUG');
  ...
  // Quick Repair: collect the FULL amount at this initial invoicing stage
  // (unlike the Full Flow spec, which splits payment 50/50) — so there is
  // no remaining balance to invoice/collect later.
  ...
  console.log(`  ✓ Collecting full payment amount: ${currentAmountStr}`);
  ...
  const clickHeaderStageAction = async (label: string, statusbarLabel: string) => {
    ...
    // Fall back to the statusbar radio in case this Odoo instance renders
    // it as directly clickable. This stage may not apply to every ticket
    // flow (e.g. a Centre/Branch repair that never leaves the branch) —
    // if the radio is also absent/disabled, skip instead of hanging until
    // the test timeout.
  ...
  // No second invoicing/payment pass needed here — the full amount was
  // already collected at the initial invoicing stage above, so there is
  // no remaining balance to invoice.
  ...
  if (!handedOverVisible) {
    ...
    if (alreadyHandedOver) {
      ...
      // A leftover technical-modal backdrop (same as the overflow-expand
      // step above) can still intercept pointer events on this final
      // click — clear it first instead of retrying against a blocked
      // click for the rest of the test timeout.
      const strayFinalModal = page.locator('.o_dialog, .modal.d-block').first();
      if (await strayFinalModal.isVisible({ timeout: 500 }).catch(() => false)) {
        const strayFinalCloseBtn = strayFinalModal.locator('[aria-label="Close"], .btn-close').first();
        if (await strayFinalCloseBtn.isVisible({ timeout: 500 }).catch(() => false)) {
          await strayFinalCloseBtn.click({ force: true }).catch(() => {});
        } else {
          await page.keyboard.press('Escape');
        }
        await waitForLoading(page);
      ...
      try {
        await handedOverBtn.click({ timeout: 8_000 });
      } catch {
        await page.keyboard.press('Escape');
        await page.waitForTimeout(300);
        await handedOverBtn.click({ force: true });
      }
      await waitForLoading(page);
